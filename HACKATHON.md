...
| Deploy fails: **`MissingSubscriptionRegistration` for `Microsoft.AlertsManagement`** | The App Insights failure-anomalies alert rule needs this provider; it's often not registered by default | `az provider register --namespace Microsoft.AlertsManagement` (wait for `Registered`), then re-run — §A.3 |
