...
| Container app revision fails on the **first** deploy: **`Operation expired` / `ContainerAppOperationError`** | First revision on a brand-new ACA environment timed out (cold environment; the orchestrator is the first app and the env is still warming) | Re-run `azd up` — the now-warm environment usually provisions the revision on retry. **But first see the next row** — a failed first deploy leaves a trap |
| **Retry of `azd up` deadlocks** on the same app (`Operation expired`, ACR has no image) | The app was created on the failed run, so azd sets `SERVICE_<APP>_RESOURCE_EXISTS=true` and Bicep points the app at `cr….azurecr.io/<app>:latest` — but that image was never built/pushed (build happens **after** provision), so the revision can't pull it. `azd env set …_EXISTS false` does **not** stick: azd re-detects the existing app and resets it to `true` each run | **Delete the half-created app(s), then re-run:** `az containerapp delete -g rg-<env> -n <app> --yes` (e.g. `orchestrator`), then `azd up`. With the app gone, azd uses the placeholder image, provisions, then builds/pushes the real image |
| Container app revision never appears; ACA system logs show **`Failed to construct registry secret … ACR token exchange endpoint returned error status: 401`** | System-assigned identity + an inline `registries` block: ACA validates the registry on every revision, but the identity's AcrPull can't be granted until the app exists → deadlock | Fixed in this branch: apps are provisioned with **no** `registries` block, AcrPull is granted, then `postprovision` attaches the registry with `--identity system`. If you re-introduce an inline registries block on a system-assigned app, you'll hit this again |
